...
async function handleHermesInit(targetDir, options) {
  ...
  if (!fromUrl) {
    ...
    console.error(chalk.gray('  Generate a setup URL at: https://shipsafe.sh/app/deploy'));
  ...
  const TRUSTED_HOSTS = ['shipsafecli.com', 'shipsafe.sh', 'localhost', '127.0.0.1'];
  if (!TRUSTED_HOSTS.includes(parsed.hostname)) {
    console.error(chalk.red('\nError: Setup URL must be from shipsafe.sh (got: ' + parsed.hostname + ')'));
    console.error(chalk.gray('  Only URLs generated by Ship Safe Cloud are trusted.\n'));
    process.exit(1);
  }
